...
def classify_song(song: Song, profile: Dict[str, object]) -> str:
    ...
    is_chill_keyword = any(k in genre for k in chill_keywords)  # Fix: was checking title, should check genre
...
def merge_playlists(a: PlaylistMap, b: PlaylistMap) -> PlaylistMap:
    ...
    for key in set(list(a.keys()) + list(b.keys())):
        merged[key] = list(a.get(key, []))  # Fix: was mutating the original list from a; now copies it first
...
def compute_playlist_stats(playlists: PlaylistMap) -> Dict[str, object]:
    ...
    total = len(all_songs)  # Fix: was len(hype), should be total song count
    hype_ratio = len(hype) / total if total > 0 else 0.0

    avg_energy = 0.0
    if all_songs:
        total_energy = sum(song.get("energy", 0) for song in all_songs)  # Fix: was summing only hype songs
...
def lucky_pick(
    ...
    if mode == "hype":
        ...
        songs = playlists.get("Hype", []) + playlists.get("Chill", []) + playlists.get("Mixed", [])  # Fix: was excluding Mixed songs from "any" mode
...
def random_choice_or_none(songs: List[Song]) -> Optional[Song]:
    ...
    return random.choice(songs) if songs else None  # Fix: was crashing with IndexError on empty list
